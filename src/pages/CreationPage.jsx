...
export default function CreationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Trigger Overlay Sequence
      setGeneratedLink(`${window.location.origin}/card/${postcardId}`);
      setShowSealingAnim(true);
      
      // Choreography Timings
      setTimeout(() => setPackStep(1), 100);    // Envelope drops in BEHIND card
      setTimeout(() => setPackStep(2), 1200);   // Flap opens
      setTimeout(() => setPackStep(3), 2000);   // Card slides UP
      setTimeout(() => setPackStep(4), 2800);   // Card slides DOWN (inside pocket)
      setTimeout(() => setPackStep(5), 3600);   // Flap closes
      setTimeout(() => setPackStep(6), 4400);   // Link UI appears
    ...
    <>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" required maxLength={30} placeholder="Recipient Name"
                  className="w-full bg-pastel-blue/5 border border-ink/10 rounded-md p-3 focus:outline-none focus:border-pastel-blue text-ink"
              ...
              <div>
                ...
                <input type="text" required maxLength={30} placeholder="Your Name"
                  className="w-full bg-pastel-blue/5 border border-ink/10 rounded-md p-3 focus:outline-none focus:border-pastel-blue text-ink"
            ...
            <div>
              ...
              <textarea required rows={5} maxLength={200} placeholder="Write something lovely..."
                className="w-full bg-pastel-blue/5 border border-ink/10 rounded-md p-3 focus:outline-none focus:border-pastel-blue font-script text-2xl resize-none text-ink"
            ...
            <div className="border-t border-ink/5 pt-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {STAMPS.map((stamp) => (
                  <button key={stamp.id} type="button" onClick={() => setFormData({...formData, stamp: stamp.img})}
                    className={clsx("flex flex-col items-center p-2 rounded-lg border-2 transition-all", formData.stamp === stamp.img ? "border-pastel-blue bg-pastel-blue/5 shadow-sm" : "border-transparent hover:bg-gray-50")}>
                    <div className="w-10 h-12 bg-gray-100 shadow-sm border border-white mb-2 overflow-hidden"><img src={stamp.img} alt={stamp.name} className="w-full h-full object-cover" /></div>
            ...
            <div className="border-t border-ink/5 pt-6">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                {FLOWERS.map((flower) => (
                  <button key={flower.id} type="button" onClick={() => setFormData({...formData, decoration: flower.img})}
                    className={clsx("flex flex-col items-center p-3 rounded-lg border-2 transition-all", formData.decoration === flower.img ? "border-pastel-pink bg-pastel-pink/10 shadow-sm" : "border-transparent hover:bg-gray-50")}>
                    <img src={flower.img} alt={flower.name} className="w-12 h-12 object-contain mb-2 drop-shadow-sm mix-blend-multiply" />
      ...
      <AnimatePresence>
        {showSealingAnim && (
          ...
          >
            {/* Fully Responsive Envelope Stage */}
            <div className="relative w-full max-w-lg aspect-[3/2] perspective-1000 mt-24">
              
              {/* Envelope Body (Starts off screen top, behind card) */}
              <motion.div 
                className="absolute inset-0 bg-[#EAE5DC] shadow-envelope rounded-b-md border border-ink/10"
              ...
              <motion.div
                ...
                animate={{ 
                  scale: 1, opacity: 1,
                  y: packStep === 3 ? '-110%' : 0, // Slide up, then down
                  zIndex: packStep >= 4 ? 10 : 50 // Drops behind pocket on step 4
                }}
                transition={{ type: "spring", stiffness: 40, damping: 15 }}
              >
              ...
              {packStep >= 6 && (
                ...
                >
                  ...
                  <div className="flex w-full gap-2 mb-6 relative z-10">
                    ...
                    <button onClick={handleCopyLink} className="bg-ink text-white px-5 py-3 rounded-lg font-semibold hover:bg-ink/90 transition-colors flex items-center gap-2">
