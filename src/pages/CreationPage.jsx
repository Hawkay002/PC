...
import { Send, Loader2, Check, Copy, ArrowRight, ArrowLeft, Inbox, Upload, Camera, Minus, Plus } from 'lucide-react';
...
// Only cssgram filters that are confirmed to work via the CDN build
const CSSGRAM_FILTERS = [
  { id: 'none',       name: 'Original',   cls: '' },
  { id: '_1977',      name: '1977',        cls: '_1977' },
  { id: 'aden',       name: 'Aden',        cls: 'aden' },
  { id: 'amaro',      name: 'Amaro',       cls: 'amaro' },
  { id: 'brannan',    name: 'Brannan',     cls: 'brannan' },
  { id: 'brooklyn',   name: 'Brooklyn',    cls: 'brooklyn' },
  { id: 'clarendon',  name: 'Clarendon',   cls: 'clarendon' },
  { id: 'earlybird',  name: 'Earlybird',   cls: 'earlybird' },
  { id: 'gingham',    name: 'Gingham',     cls: 'gingham' },
  { id: 'hudson',     name: 'Hudson',      cls: 'hudson' },
  { id: 'inkwell',    name: 'Inkwell',     cls: 'inkwell' },
  { id: 'kelvin',     name: 'Kelvin',      cls: 'kelvin' },
  { id: 'lark',       name: 'Lark',        cls: 'lark' },
  { id: 'lofi',       name: 'Lo-Fi',       cls: 'lofi' },
  { id: 'maven',      name: 'Maven',       cls: 'maven' },
  { id: 'mayfair',    name: 'Mayfair',     cls: 'mayfair' },
  { id: 'moon',       name: 'Moon',        cls: 'moon' },
  { id: 'nashville',  name: 'Nashville',   cls: 'nashville' },
  { id: 'perpetua',   name: 'Perpetua',    cls: 'perpetua' },
  { id: 'reyes',      name: 'Reyes',       cls: 'reyes' },
  { id: 'rise',       name: 'Rise',        cls: 'rise' },
  { id: 'sierra',     name: 'Sierra',      cls: 'sierra' },
  { id: 'slumber',    name: 'Slumber',     cls: 'slumber' },
  { id: 'toaster',    name: 'Toaster',     cls: 'toaster' },
  { id: 'valencia',   name: 'Valencia',    cls: 'valencia' },
  { id: 'walden',     name: 'Walden',      cls: 'walden' },
  { id: 'willow',     name: 'Willow',      cls: 'willow' },
  { id: 'xpro2',      name: 'X-Pro II',    cls: 'xpro2' },
];
...
export default function CreationPage() {
  ...
  const [fontSizeIdx, setFontSizeIdx] = useState(2); // default 'M' = text-2xl
  ...
  const submitPostcard = async () => {
    ...
    try {
      ...
      const postcardId = await createPostcard({ ...formData, font_size: FONT_SIZES[fontSizeIdx].value, file_id: uploadData.file_id });
      ...
      // Animation timing: envelope slides in (step 1), then flap opens (step 2),
      // postcard rises out (step 3). z-index swap (step 4) happens AFTER
      // the postcard has fully cleared the envelope (~800ms rise + buffer).
      setTimeout(() => setPackStep(1), 100);   // envelope enters
      setTimeout(() => setPackStep(2), 1400);  // flap starts opening
      setTimeout(() => setPackStep(3), 2400);  // postcard rises
      setTimeout(() => setPackStep(4), 3600);  // flap z-index swap (postcard fully out)
      setTimeout(() => setPackStep(5), 4200);  // flap closes
      setTimeout(() => setPackStep(6), 5000);  // seal appears + success card
    } catch (error) {
      console.error(error);
  ...
  const labelClass = "text-xs font-sans font-medium uppercase tracking-[0.15em] text-muted";
    ...
    <>
      <div className="bg-obsidian bg-noise h-[100dvh] overflow-hidden flex flex-col">
        ...
        <div className="shrink-0 flex items-center justify-between px-5 lg:px-8 py-4 border-b border-rim/50">
          ...
          <Link to="/dashboard" className="flex items-center gap-2 text-xs font-sans text-muted hover:text-champagne border border-rim hover:border-gold/30 transition-all px-3.5 py-1.5 rounded-sm">
        ...
        <div className="max-w-7xl mx-auto w-full flex-1 min-h-0 p-4 lg:p-6 flex flex-col lg:flex-row gap-4 lg:gap-6">
          ...
          <div className="w-full lg:w-[45%] shrink-0 flex flex-col items-center justify-center z-20 pb-3 border-b lg:border-none border-rim/30">
            <div className="w-full max-w-md">
              ...
              <Postcard
                data={{ ...formData, previewUrl, font_size: FONT_SIZES[fontSizeIdx].value }}
                isInteractive={true}
                forceFlip={currentStep === 2}
                onRemoveImage={handleRemoveImage}
              />
              <p className="text-xs text-muted/60 text-center mt-3 font-sans font-light italic">Tap to flip</p>
            </div>
          </div>
          ...
          <div className="w-full lg:w-[55%] bg-panel border border-rim/50 rounded-sm flex flex-col flex-1 min-h-0 overflow-hidden">
                ...
                <div key={step.num} className="flex items-center flex-1">
                  <button
                    ...
                    className={clsx(
                      ...
                      currentStep === step.num ? 'text-champagne'
                        : currentStep > step.num ? 'text-gold/60' : 'text-muted/50'
                    )}
                  >
                    <span className="text-[10px] font-sans uppercase tracking-[0.15em] font-medium">{step.label}</span>
                    {currentStep === step.num && (
                      <motion.div layoutId="step-underline" className="absolute bottom-0 left-2 right-2 h-px bg-gradient-to-r from-transparent via-gold to-transparent" />
                    )}
                    {currentStep > step.num && (
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin">
              <div className="p-5 lg:p-7">
                ...
                {/* ── STEP 1: MESSAGE ── */}
                {currentStep === 1 && (
                  <motion.div key="step1" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-5">
                    <div>
                      <h2 className="font-display text-3xl lg:text-4xl font-light italic text-luminary mb-1">Compose your letter</h2>
                      <p className="text-muted text-xs font-sans">Every great postcard starts with heartfelt words.</p>
                    </div>
                    <div className="w-8 h-px bg-gold/40" />
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className={`block ${labelClass} mb-2`}>To</label>
                        <input type="text" maxLength={30} placeholder="Recipient" className={inputClass}
                          value={formData.to} onChange={e => setFormData({ ...formData, to: e.target.value })} />
                      </div>
                      <div>
                        <label className={`block ${labelClass} mb-2`}>From</label>
                        <input type="text" maxLength={30} placeholder="Your name" className={inputClass}
                          value={formData.from} onChange={e => setFormData({ ...formData, from: e.target.value })} />
                      </div>
                    </div>
                    <div>
                      {/* Label row with inline font-size stepper */}
                      <div className="flex items-center justify-between mb-2">
                        <label className={labelClass}>Message</label>
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] font-sans text-muted uppercase tracking-widest mr-1">Size</span>
                          <button
                            type="button"
                            onClick={() => setFontSizeIdx(i => Math.max(0, i - 1))}
                            disabled={fontSizeIdx === 0}
                            className="w-6 h-6 flex items-center justify-center border border-rim hover:border-gold/40 rounded-sm text-muted hover:text-champagne disabled:opacity-30 transition-all"
                          >
                            <Minus className="w-3 h-3" />
                          </button>
                          <span className="text-[10px] font-sans text-champagne w-4 text-center">{FONT_SIZES[fontSizeIdx].label}</span>
                          <button
                            type="button"
                      ...
                      <textarea
                        ...
                        className={`${inputClass} font-script resize-none leading-[1.6] ${FONT_SIZES[fontSizeIdx].value}`}
                ...
                {/* ── STEP 2: PHOTO ── */}
                {currentStep === 2 && (
                  <motion.div key="step2" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-5">
                    <div>
                      <h2 className="font-display text-3xl lg:text-4xl font-light italic text-luminary mb-1">Attach a memory</h2>
                      <p className="text-muted text-xs font-sans">A photograph for the back of your postcard.</p>
                    </div>
                    <div className="w-8 h-px bg-gold/40" />

                    {/* Slimmer upload/camera row */}
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        type="button" onClick={() => fileInputRef.current?.click()}
                        className="flex items-center justify-center gap-2.5 border border-rim hover:border-gold/40 bg-charcoal/50 hover:bg-surface py-3 px-4 rounded-sm transition-all group"
                      >
                        <Upload className="w-4 h-4 text-muted group-hover:text-gold transition-colors shrink-0" />
                        <span className="text-xs font-sans uppercase tracking-[0.12em] text-muted group-hover:text-champagne">Upload</span>
                      </button>
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-2.5 border border-rim hover:border-gold/40 bg-charcoal/50 hover:bg-surface py-3 px-4 rounded-sm transition-all group"
                      >
                        <Camera className="w-4 h-4 text-muted group-hover:text-gold transition-colors shrink-0" />
                        <span className="text-xs font-sans uppercase tracking-[0.12em] text-muted group-hover:text-champagne">Camera</span>
                      </button>
                    ...
                    {/* Filter strip — full width, no clipping */}
                    <div className={clsx(
                      ...
                      <p className={`${labelClass} mb-3 block`}>Film Filter</p>
                      {/* Scrollable filter row — negative margin so it bleeds to panel edges */}
                      <div
                        className="-mx-5 lg:-mx-7 px-5 lg:px-7 flex gap-2 overflow-x-auto pb-3"
                      ...
                      >
                        {CSSGRAM_FILTERS.map((f) => (
                          <button
                            key={f.id}
                            type="button"
                            onClick={() => setFormData({ ...formData, image_filter: f.cls })}
                            className={clsx(
                              "shrink-0 flex flex-col items-center gap-1.5 p-2 transition-all rounded-sm w-[68px]",
                              formData.image_filter === f.cls
                                ? "bg-gold/10 ring-1 ring-gold/50"
                                : "hover:bg-surface"
                            )}
                          >
                            <figure className={clsx(
                              "w-11 h-11 rounded-full overflow-hidden m-0 ring-1",
                              formData.image_filter === f.cls ? "ring-gold/60" : "ring-rim",
                              f.cls   // cssgram class applied to <figure>
                            )}>
                              <img
                            ...
                            <span className={clsx(
                              "text-[9px] font-sans uppercase tracking-wide text-center leading-tight",
                              formData.image_filter === f.cls ? "text-champagne" : "text-muted"
                            )}>{f.name}</span>
                          </button>
                        ))}
                      </div>
                ...
                {/* ── STEP 3: STAMP ── */}
                {currentStep === 3 && (
                  <motion.div key="step3" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-5">
                    <div>
                      <h2 className="font-display text-3xl lg:text-4xl font-light italic text-luminary mb-1">Choose a stamp</h2>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {STAMPS.map((stamp) => (
                        ...
                        >
                          ...
                          <span className={clsx("text-xs font-sans text-center", formData.stamp === stamp.img ? "text-champagne" : "text-muted")}>
                            {stamp.name}
                          </span>
                        </button>
                      ))}
                ...
                {/* ── STEP 4: FLORALS ── */}
                {currentStep === 4 && (
                  <motion.div key="step4" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-5">
                    <div>
                      <h2 className="font-display text-3xl lg:text-4xl font-light italic text-luminary mb-1">Floral accent</h2>
                      <p className="text-muted text-xs font-sans">Adorn your postcard with a botanical illustration.</p>
                    </div>
                    <div className="w-8 h-px bg-gold/40" />
                    {/* Same glowy card style as stamps */}
                    <div className="grid grid-cols-2 gap-3">
                      {FLOWERS.map((flower) => (
                        <button
                          key={flower.id} type="button"
                        ...
                        >
                          {/* Cream circle thumbnail */}
                          <div className="w-12 h-12 rounded-full bg-cream shrink-0 flex items-center justify-center">
                            <img src={flower.img} alt={flower.name} className="w-10 h-10 object-contain mix-blend-multiply" />
                          </div>
                          <span className={clsx(
            ...
            <div className="shrink-0 flex items-center justify-between px-5 lg:px-7 py-4 border-t border-rim/50">
              ...
              {currentStep < 4 ? (
                ...
                <button type="button" onClick={submitPostcard} disabled={isSubmitting}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="bg-panel border border-rim rounded-sm w-full max-w-2xl overflow-hidden flex flex-col h-[80vh]">
              <div className="p-4 border-b border-rim flex justify-between items-center">
                ...
                <button onClick={() => setRawImageSrc(null)} className="text-muted hover:text-luminary text-sm font-sans transition-colors">Cancel</button>
              </div>
              <div className="flex-1 relative bg-obsidian">
                <Cropper image={rawImageSrc} crop={crop} zoom={zoom} aspect={3/2}
      ...
      <AnimatePresence>
        {showSealingAnim && (
          <motion.div key="animation-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <div className="relative w-full max-w-lg aspect-[3/2] perspective-1000 mb-32">
              ...
              <motion.div
                ...
                style={{ zIndex: packStep >= 4 ? 10 : 50 }}
                initial={{ scale: 0.88, opacity: 0 }}
                animate={{
                  scale: 0.88,
                  opacity: packStep >= 1 ? 1 : 0,
                  y: packStep >= 3 ? '-115%' : 0,
                }}
                transition={{ type: 'spring', stiffness: 35, damping: 14 }}
              >
                <div className="w-full">
                  <Postcard data={{ ...formData, previewUrl, font_size: FONT_SIZES[fontSizeIdx].value }} isInteractive={false} forceFlip={false} />
                </div>
              </motion.div>

              {/* Bottom flap (static, z-20) */}
              ...
              {/* Top flap — z-index swaps AFTER postcard fully exits (packStep 4) */}
              <motion.div
                className="absolute top-0 left-0 w-full h-full origin-top"
                style={{ zIndex: packStep >= 4 ? 0 : 30 }}
                animate={{ rotateX: (packStep >= 2 && packStep < 5) ? 180 : 0 }}
              ...
              {/* Wax seal on envelope after animation */}
              <motion.img src="/seal-1.webp" alt="Wax Seal"
