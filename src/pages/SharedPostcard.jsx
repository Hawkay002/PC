...
export default function SharedPostcard() {
  ...
  useEffect(() => {
    async function load() {
      try { setData(await getPostcard(id)); }
  ...
  if (error) return (
    <div className="min-h-screen bg-obsidian bg-noise flex flex-col items-center justify-center space-y-4 px-4 text-center">
      ...
      <Link to="/" className="text-xs font-sans uppercase tracking-[0.15em] text-gold/70 hover:text-gold border border-gold/20 hover:border-gold/50 px-5 py-2 rounded-sm transition-all">
        Send a new one
      </Link>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-obsidian bg-noise flex flex-col items-center justify-center overflow-hidden relative">
      ...
      {/* No drop-shadow on the Envelope wrapper */}
      <div className="w-full" style={{ filter: 'none' }}>
        <Envelope postcardData={data} />
      </div>
      <div className="fixed bottom-8 text-center z-10">
        <Link to="/"
          className="inline-flex items-center gap-2 bg-panel/80 backdrop-blur-md border border-gold/20 hover:border-gold/50 text-champagne px-5 py-2.5 rounded-sm text-xs font-sans uppercase tracking-[0.15em] transition-all">
