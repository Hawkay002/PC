...
import { deletePostcard } from '../lib/supabase'; // <-- Import added
...
export default function Dashboard() {
  ...
  const handleDelete = async (idToDelete) => {
    ...
    try {
      ...
      await deletePostcard(idToDelete);
      ...
      alert("Could not delete the postcard.");
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 lg:p-12 min-h-screen flex flex-col">
      <header className="flex items-center justify-between mb-12">
        ...
        <Link
          to="/"
          className="bg-ink text-white px-5 py-2.5 rounded-lg font-semibold flex items-center gap-2 hover:bg-ink/90 transition-colors shadow-sm"
        >
      ...
      {postcards.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center bg-white/50 rounded-2xl border border-ink/5 p-12 text-center">
          ...
          <p className="text-ink/60 mb-6">
            You haven't sent any digital postcards yet.
          </p>

          <Link to="/" className="text-ink underline font-medium">
        ...
        <div className="space-y-4">
          {postcards.map((card) => (
            <div
              key={card.id}
              className="bg-white p-5 rounded-xl border border-ink/5 shadow-sm flex items-center justify-between group hover:border-pastel-blue transition-colors"
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <Link
                  to={`/card/${card.id}`}
                  className="w-10 h-10 flex items-center justify-center bg-pastel-blue/20 text-ink rounded-full group-hover:bg-pastel-blue group-hover:text-white transition-all"
                >
